...
* Интерпретатор URQL:
	...
	+ Добавлено: Оператор возведения в степень `^` (Право-ассоциативный).
	+ Добавлено: Составные операторы присваивания, такие как `+=`, `-=`, `*=` и `/=`.
	+ Добавлено: Системные переменные:
		...
		- file_url - получить относительный URL-адрес файла из пакета игры по его внутреннему пути.
		- hide_use - скрыть или показать все варианты действий всех предметов инвентаря.
		- image_caption - установить или получить видимую подпись к изображению.
		- instr_leave_spc - установить или получить режим обработки пробелов для оператора instr.
		- last_btn_caption - получить название последней активированной кнопки выбора или ссылки в тексте.
		- location - получить имя текущей локации, вне зависимости от способа перехода в эту локацию (только для режима совместимости с AkURQ).
		- quest_path - всегда имеет значение в виде пустой строки (только для режима совместимости с AkURQ).
		- time - получить текущее время.
		- urq_inv - получить количество предметов в инвентаре.
		- urq_type - всегда имеет значение 1 (только для режима совместимости с AkURQ).
	...
	+ Добавлено: Оператор varkill для удаления определённой переменной или всех переменных.
	...
	+ Добавлено: Более надёжная загрузка интерпретируемых файлов игры в кодировке Windows-1251 при больших объёмах данных.
	+ Добавлено: Режим совместимости с AkURQ.
	+ Добавлено: Автоматическое экранирование HTML (включено по умолчанию в режимах совместимости с RipURQ и URQ_DOS).
	+ Изменено: Оператор clsb теперь только удаляет кнопки, но не очищает текст от ссылок.
	+ Изменено: Любое ненулевое число или непустая строка в булевом контексте (например, в условии оператора `if`) теперь интерпретируется как true.
	+ Изменено: Значения системных переменных `hide_*` и `instr_leave_spc` теперь неявно приводятся к булевому значению во время оценки согласно стандартным правилам приведения типов URQL.
	+ Изменено: Инициализация переменной с помощью оператора `instr` без присваивания значения теперь приводит к тому, что переменная инициализируется со значением в виде пустой строки, а не игнорируется.
	+ Изменено: При присваивании строкового значения переменной с помощью оператора instr ведущие и конечные пробельные символы в строке значения теперь удаляются. Вернуть прежнее поведение с сохранением пробелов можно с помощью системной переменной instr_leave_spc.
	+ Исправлено: Некорректная обработка унарных операторов `-` и `+` слева от переменных и скобок.
	...
	+ Исправлено: Если при присвоении значения системным переменным image или music перед знаком равно есть пробел, то конструкция ошибочно интерпретируется как выполнение одноимённых операторов с некорректным путём к файлу.
	+ Исправлено: Оператор not сразу после левой скобки обрабатывается некорректно.
	+ Исправлено: Некорректная обработка переменной tokens_delim.
	+ Исправлено: Некорректная обработка оператора goto в режиме совместимости с URQ_DOS.
* Плеер игр:
	+ Добавлено: Поддержка настраиваемой интернационализации и переводы интерфейса на русский, английский, армянский, белорусский, испанский, межславянский (латиница), украинский и эсперанто языки.
	+ Добавлено: Дифференцированная языковая разметка, соответствующая языку интерфейса и языку игрового контента.
	...
	+ Добавлено: Игры, добавленные в каталог, имеют отдельные URL-адреса для идентификации и индексации поисковыми системами.
	+ Добавлено: Тег с каноническим URL-адресом (отдельные URL-адреса для главной страницы с каталогом и для каждой игры, запускаемой по идентификатору).
	+ Добавлено: Карта сайта для поисковых систем.
	+ Добавлено: Возможность задать игру, загружаемую при любом открытии index.html или index-dev.html.
	+ Добавлено: Новые ключи объекта игры в файле games.json:
		- date - для указания даты первой публикации игры.
		- lang - для указания одного или нескольких языков игры.
	+ Добавлено: Возможность задать настройки плеера с помощью get-параметров:
		- Идентификатор игры для запуска - id (значение: идентификатор).
		...
		- URL для открытия игры - url (значение: строка URL-адреса).
	...
	+ Добавлено: Возможность сворачивать и развлорачивать разделы на главной странице.
	+ Добавлено: Отображение подробной информации об UrqW на главной странице.
	+ Добавлено: Отображение версии UrqW в интерфейсе плеера.
	+ Добавлено: Отображение информации о неподдерживаемом формате игр и о процессе распаковки загруженной игры.
	+ Добавлено: Заголовки в диалоговых окнах сохранения и загрузки игры.
	+ Добавлено: Поддержка открытия игр в форматах .qs1, .qs2 и .qsz.
	+ Изменено: Названия предметов в инвентаре, формируемые из названий переменных, теперь отображаются с заглавной буквы и без пробелов на концах.
	+ Изменено: Варианты использования предмета в инвенторе теперь отображаются, когда предмет принимает системный фокус, а не только когда на него наводится мышь.
	+ Изменено: Названия вариантов использования предмета в инвенторе, формируемые из названий меток использования, теперь отображаются с пробелами вместо подчёркиваний и без пробелов на концах.
	+ Изменено: При окончании игры через оператор quit ссылки и кнопки теперь удаляются, а не просто перестают реагировать на действия пользователя.
	+ Изменено: При запуске игры по идентификатору предпочтение теперь отдаётся идентификатору игры из get-параметра id (сохранена поддержка запуска игры по идентификатору фрагмента (хэшу), но только если get-параметр id отсутствует).
	+ Изменено: При загрузке игры из архива теперь как плагины читаются и загружаются файлы с названиями style.css и script.js (как документировано), а не все файлы .css и .js (как было фактически).
	+ Изменено: При запуске игры по идентификатору из папки плеер теперь ищет файл urqw/main.qst и, если не найден, то main.qst вместо файла quest.qst. Директория расположения main.qst считается директорией игры.
	+ Изменено: При запуске игры по идентификатору из папки теперь сначала загружаются и обрабатываются файлы manifest.json и записи iFiction, а также файлы плагинов (style.css и script.js), и только после этого файл игры main.qst.
	+ Изменено: Новый звук, воспроизводимый при помощи оператора play, теперь останавливает предыдущий (оригинальное поведение RipURQ, URQ_DOS (WCL) и AkURQ), а не воспроизводится параллельно.
	+ Изменено: Звуки игры, инициированные посредством оператора play при нулевой громкости, теперь начинают воспроизводиться с нулевой громкостью, а не игнорируются.
	+ Изменено: Области интерфейса для текста и кнопок выбора теперь перерисовываются только при изменении отображаемого содержимого, а не при каждом изменении состояния игры.
	+ Изменено: Теперь отслеживание нажатий клавиш не по коду символа, а по коду клавиши.
	+ Изменено: Оптимизация вёрстки, в том числе для валидности HTML и поисковой оптимизации.
	...
	+ Улучшения доступности (accessibility):
		...
		- Изменено: Каждая ссылка в выпадающем меню элемента инвентаря теперь имеет роль пункта меню.
...
* Инфраструктура разработки и сопровождения:
	+ Добавлено: Автоматизация сборки и развёртывания UrqW с каталогом игр через GitHub Actions на GitHub Pages при изменениях в master.
	+ Добавлено: Скрипт build-urqw для сборки продакшен-версии движка.
	...
	+ Добавлено: Скрипт gen-sitemap для генерации карты сайта для поисковых систем.
	...
	+ Добавлено: Расширенное тестовое покрытие функциональности плеера игр и конструкций языка программирования.
	+ Изменено: Для продакшен-сборок движка се подключённые отдельно .css файлы стилей и .js файлы скриптов теперь минифицируются и объединяются в единые пакеты, а также минифицируется файл index.html. Разработка ведётся в отдельных .css и .js файлах, а также в файле index-dev.html.
	+ Изменено: Все игры в каталоге теперь добавлены как подмодули из отдельных репозиториев и перепакованы с файлами manifest.json и записи iFiction.
	+ Изменено: Поддерживаемые плагины с примерами использования (функциональными тестами) теперь добавлены как подмодули из отдельных репозиториев и размещены в директории quests как игровые пакеты.
