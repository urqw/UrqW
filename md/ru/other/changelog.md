...
* Интерпретатор URQL:
	...
	+ Добавлено: Системные переменные image_caption, time, date (только при отсутствии специальных правил urq_mode), urqw_title, urqw_game_ifid, urqw_game_lang, urqw_version, quest_path (только для режима совместимости с AkURQ).
...
* Плеер игр:
	...
	+ Изменено: При запуске игры по идентификатору фрагмента (хэшу) из папки сначала загружаются и обрабатываются файлы manifest.json и записи iFiction, а также файлы плагинов (script.js и style.css), и только после этого файл игры main.qst.
