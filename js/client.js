...
Client.prototype.drawItem = function (itemName, quantity) {
    ...
    if (displayName) {
        itemDisplayName = displayName;
                ...
                itemDisplayName = itemName.charAt(0).toUpperCase() + itemName.slice(1);
                itemDisplayName = itemDisplayName.replace(/_/g, ' ');
                itemDisplayName = itemDisplayName.trim();
    }
    if (!htmlSupport) {
        // <br> tag can be generated by #/$ construct from URQL code
        ...
        for (var i = 0; i < actions.length; i++) {
            ...
            if (!toBoolean(Game.getVar('hide_use_' + itemName + '_' + actions[i][0]))) {
                ...
                if (displayName) {
                    ...
                    actionDisplayName = actionDisplayName.trim();
                }
