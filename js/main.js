...
$(function() {
    ...
    function loadFromHashFailed() {
        $.ajax({
            ...
            $('.gamelist').text('Не удалось загрузить список квестов. Скорее всего, у вас браузер на основе Chromium (Chrome, Яндекс Браузер, Opera и др.), и вы запустили UrqW локально. Безопасность Chromium запрещает обращаться к каким бы то ни было локальным файлам и считывать их автоматически. Это исправляется, если запустить Chrome с флагом "--allow-file-access-from-files". В вебе такой проблемы ни у кого не будет, речь только о локальной работе. Вы всё ещё можете выбрать файлы игры вручную из папки quests и поиграть.')
        });
